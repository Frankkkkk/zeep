...
impl<'n> TryFromNode<'n> for Field {
    ...
    fn try_from_node(node: Node<'n, 'n>, doc: &mut RustDocument) -> WriterResult<Self> {
        ...
        // override. Per the XSD spec both defaults are "unqualified" -- a
        // generator that always qualifies local fields produces XML that
        // doesn't validate against the schema it claims to implement.
        let is_qualified = match node.attribute("form") {
            ...
            // qualified -- this pattern is used for synthesized/global
